...
const SearchPage = () => {
  ...
  const handleReanalyzeAll = useCallback(async () => {
    ...
    try {
      ...
      if (failureCount === 0) toast.success(`Re-analyzed ${successCount} files`);
      else toast.warning(`Re-analyzed ${successCount} files, ${failureCount} failed`);
    } catch { toast.error("Bulk re-analyze failed"); }
    finally {
