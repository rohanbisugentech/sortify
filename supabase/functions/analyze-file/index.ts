...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert document analysis AI for Sortify. Extract MAXIMUM useful metadata.
...
CRITICAL: "extracted_text" must contain ALL key text verbatim. Include dates in multiple formats.`,
    ...
    const model = useVisionModel ? "gemini-2.5-flash" : "gemini-3-flash-preview";
